...
namespace App\Anser\Services\PaymentService\V2;
...
class Payment extends SimpleService
{
    ...
    public function getPaymentList(
        ...
        $action = $this->getAction("GET", "/api/v2/payments");
        if(!empty($payload)) {
    ...
    {
        $action = $this->getAction("GET", "/api/v2/payments/{$paymentKey}")
            ->addOption("headers", [
                "X-User-key" => 1
            ])
    ...
    {
        $action = $this->getAction("POST", "/api/v2/payments")
    ...
    {
        $action = $this->getAction("PUT", "/api/v2/payments")
            ->addOption("headers", [
                "X-User-key" => 1
            ])
            ->addOption("json", [
     ...
     * @return ActionInterface
     */
    public function deletePayment(string $orderKey, int $userKey): ActionInterface
    {
        $action = $this->getAction("DELETE", "/api/v2/payments/{$orderKey}")
            ->addOption("headers", [
                "X-User-key" => $userKey
            ])
            ->doneHandler(function (
